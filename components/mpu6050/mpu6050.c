...
esp_err_t mpu6050_write(mpu6050_handle_t sensor, const uint8_t reg_start_addr, const uint8_t *const data_buf, const uint8_t data_len)
{
    mpu6050_dev_t *sens = (mpu6050_dev_t *)sensor;
    esp_err_t ret = ESP_OK;
        i2c_master_bus_handle_t bus_handle;
    ESP_ERROR_CHECK(i2c_master_get_bus_handle(sens->bus, &bus_handle));
    i2c_device_config_t dev_cfg = {
        .dev_addr_length = I2C_ADDR_BIT_LEN_7,
        .device_address = sens->dev_addr,
    ...
    uint8_t cmd_buff[20];
    int8_t data_ptr = 0;
    cmd_buff[data_ptr] = reg_start_addr;
    for (int8_t i = 0; i < data_len; ++ i){
        data_ptr += 1;
        cmd_buff[data_ptr] = data_buf[i];
    }
    // printf("reg: 0x%02X -> ", cmd_buff[0]);
    // for (int8_t i = 1; i < data_len + 1; ++ i){
    //     printf("0x%02X ", cmd_buff[i]);
    // }
    // printf("\n");
    ret = i2c_master_transmit(dev_handle, cmd_buff, data_len+1, 1000);
    ESP_ERROR_CHECK(i2c_master_bus_rm_device(dev_handle));
    return ret;
}

esp_err_t mpu6050_read(mpu6050_handle_t sensor, const uint8_t reg_start_addr, uint8_t *const data_buf, const uint8_t data_len)
{
    mpu6050_dev_t *sens = (mpu6050_dev_t *) sensor;
    esp_err_t  ret = ESP_OK;
    i2c_master_bus_handle_t bus_handle;
    ...
    i2c_master_dev_handle_t dev_handle;
    ESP_ERROR_CHECK(i2c_master_bus_add_device(bus_handle, &dev_cfg, &dev_handle));
    ret = i2c_master_transmit_receive(dev_handle, &reg_start_addr, sizeof(uint8_t), data_buf, data_len, 1000);
    ESP_ERROR_CHECK(i2c_master_bus_rm_device(dev_handle));
    return ret;
}

mpu6050_handle_t mpu6050_create(i2c_port_num_t port, const uint16_t dev_addr)
{
    mpu6050_dev_t *sensor = (mpu6050_dev_t *) calloc(1, sizeof(mpu6050_dev_t));
    sensor->bus = port;  // ic接口号
...
esp_err_t mpu6050_config(mpu6050_handle_t sensor, const mpu6050_acce_fs_t acce_fs, const mpu6050_gyro_fs_t gyro_fs)
{
    ...
    uint8_t cmd[3];
    uint8_t reg;
    uint8_t delay_time_ms = 0;
    reg = MPU6050_PWR_MGMT_1, cmd[0] = 0x00;
    mpu6050_write(sensor, reg, cmd, 1);
    vTaskDelay(pdMS_TO_TICKS(delay_time_ms));
    reg = MPU6050_SMPLRT_DIV, cmd[0] = 0x07;
    mpu6050_write(sensor, reg, cmd, 1);
    vTaskDelay(pdMS_TO_TICKS(delay_time_ms));
    reg = MPU6050_CONFIG, cmd[0] = 0x04;
    mpu6050_write(sensor, reg, cmd, 1);
    vTaskDelay(pdMS_TO_TICKS(delay_time_ms));
    reg = MPU6050_GYRO_CONFIG, cmd[0] = gyro_fs << 3;
    mpu6050_write(sensor, reg, cmd, 1);
    vTaskDelay(pdMS_TO_TICKS(delay_time_ms));
    reg = MPU6050_ACCEL_CONFIG, cmd[0] = acce_fs << 3;
    mpu6050_write(sensor, reg, cmd, 1);
    vTaskDelay(pdMS_TO_TICKS(delay_time_ms));
    reg = MPU6050_USER_CONTROL, cmd[0] = 0x00;
    mpu6050_write(sensor, reg, cmd, 1);
    vTaskDelay(pdMS_TO_TICKS(delay_time_ms));
    reg = MPU6050_INT_PIN_CFG, cmd[0] = 0x02;
    mpu6050_write(sensor, reg, cmd, 1);
    vTaskDelay(pdMS_TO_TICKS(delay_time_ms));
    return ESP_OK;
}
...
esp_err_t mpu6050_get_acce(mpu6050_handle_t sensor, mpu6050_acce_value_t *const acce_value)
{
    ...
    // printf("acce_sensitivity:%.2f\n", acce_sensitivity);
    if (ret != ESP_OK) {
        return ret;
    }
    ret = mpu6050_get_raw_acce(sensor, &raw_acce);
    // ESP_LOGI("MPU6050 RAW", "acce_x:%d, acce_y:%d, acce_z:%d\n", raw_acce.raw_acce_x, raw_acce.raw_acce_y, raw_acce.raw_acce_z);
    if (ret != ESP_OK) {
        return ret;
    }
